...
Or build the docker container and do the same:

```bash
docker build --file container/Dockerfile --taget=image --tag=anserno .
docker run --mount /path/to/library:/library --library-url file:///library
```
