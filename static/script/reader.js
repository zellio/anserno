...
$(window).ready((event) => {
  ...
  epub_reader.book.ready.then((event) => {
    ...
    $("#toc").html($('<div/>', {
      ...
      html: $("<select/>", {
        id: "toc-selector",
        class: "",
        html: spine.items.map((spine_element, index, array) => {
          let spine_href = decodeURIComponent(spine_element.href);

          let toc_element = navigation.toc.find((toc_element, index, array) => {
  ...
  epub_reader.rendition.hooks.content.hooks.push((contents, view) => {
